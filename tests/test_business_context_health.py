...
from src.services.business_context_health import (
    ...
    should_monitor_business_context_health,
)
...
def test_should_monitor_only_current_client_projects():
    current_truth_docs = [
        make_doc(
            "clients/xyz/requirements.md",
            {
    ...
    assert should_monitor_business_context_health(
        make_project(
            name="XYZ current",
            description="Current client business context.",
            business_lifecycle="ACTIVE_CLIENT",
        ),
        current_truth_docs,
    )
    assert not should_monitor_business_context_health(
        make_project(
            name="ACME archive",
            description="Historical client reference context.",
            business_lifecycle="REFERENCE_ARCHIVE",
        ),
        current_truth_docs,
    )
    assert not should_monitor_business_context_health(
        make_project(
            ...
            description="Internal workspace context.",
            business_lifecycle="INTERNAL_BUSINESS",
        ),
        current_truth_docs,
    )


def test_should_not_monitor_legacy_reference_only_projects():
    assert not should_monitor_business_context_health(
        make_project(name="Legacy archive", description="Client business context."),
        [
            make_doc(
                "clients/acme/diagram.svg",
                {
                    "assetClass": "DIAGRAM",
                    "usageMode": "historical_reference",
                    "clientId": "acme",
...
@pytest.mark.asyncio
async def test_index_recommendations_skip_business_health_for_archived_projects():
    health = IndexHealth(
        total_documents=1,
        indexed_documents=1,
        unindexed_documents=0,
    ...
    assert all(not item["type"].startswith("business_context_") for item in recommendations)
