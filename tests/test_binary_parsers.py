...
def test_svg_parser_accepts_base64_payload():
    parser = SvgDocumentParser()

    result = parser.parse(
        content=_base64_payload(SVG_SAMPLE.encode("utf-8")),
...
def test_unparseable_binary_document_is_skipped():
    indexer = DocumentIndexer(AsyncMock())
    ...
    assert indexer._is_chunkable_document(invalid_doc) is True
    assert indexer._get_indexable_content(invalid_doc) is None
