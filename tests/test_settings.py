...
try:
    ...
    class ProjectSettings:
        ...
        memory_novelty_threshold: float | None = 0.92

        def __post_init__(self):
            if self.memory_novelty_threshold is None:
                self.memory_novelty_threshold = 0.92
...
class TestProjectSettings:
    ...
    def test_null_memory_novelty_threshold_uses_default(self):
        """Test null novelty threshold is normalized to the default."""
        settings = ProjectSettings(memory_novelty_threshold=None)
        assert settings.memory_novelty_threshold == 0.92
...
class TestRLMEngineSettings:
    ...
    def test_null_novelty_threshold_uses_default(self):
        """Test engine normalizes null novelty threshold from persisted settings."""
        engine = RLMEngine("test-project", settings={"memory_novelty_threshold": None})
        assert engine.settings.memory_novelty_threshold == 0.92
