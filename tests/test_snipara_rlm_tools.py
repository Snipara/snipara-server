...
@pytest.mark.asyncio
async def test_call_tool_prefers_remote_v1_endpoint():
    """Remote API calls should target the v1 MCP endpoint first."""
...
def test_get_snipara_tools_includes_full_backend_contract(monkeypatch: pytest.MonkeyPatch):
    """rlm-runtime integration should expose every remote rlm_* tool."""
...
async def test_generated_contract_tool_handler_forwards_arguments(monkeypatch: pytest.MonkeyPatch):
    """Generated rlm_* handlers should forward kwargs verbatim to the remote API."""
    _install_fake_rlm(monkeypatch)
