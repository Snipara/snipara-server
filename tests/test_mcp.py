...
class TestToolDefinitions:
    ...
    def test_context_query_tool_exists(self):
        ...
        assert "times out" in tool["description"]
        assert "return_references=true" in tool["description"]
        assert "rlm_code_neighbors" in tool["description"]
    ...
    def test_htask_tree_accepts_optional_task_id(self):
        """rlm_htask_tree should support both all-roots and rooted tree calls."""
        tool = next((t for t in TOOL_DEFINITIONS if t["name"] == "rlm_htask_tree"), None)

        assert tool is not None
        schema = tool["inputSchema"]
        assert schema["required"] == ["swarm_id"]
        assert "task_id" in schema["properties"]
        assert "optional" in schema["properties"]["task_id"]["description"].lower()
