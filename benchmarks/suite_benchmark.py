...
from .config import TOKEN_COSTS, load_oauth_access_token
...
class SuiteConfig:
    ...
    # Snipara
    snipara_project_slug: str = "snipara"
    snipara_oauth_token: Optional[str] = field(default_factory=load_oauth_access_token)
...
async def run_benchmark(cfg: SuiteConfig, suites: list[str], modes: list[str]):
    ...
    if any(m != "llm_alone" for m in modes):
        try:
            ...
            print(f"Snipara client: OAuth ({'token loaded' if cfg.snipara_oauth_token else 'no token'})")
        except Exception as e:
