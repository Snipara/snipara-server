...
def resolve_snipara_project_ref(default: str = "snipara") -> str:
    """Resolve the project identifier for benchmark runs."""
    return (
        os.getenv("SNIPARA_PROJECT_SLUG")
        or os.getenv("SNIPARA_PROJECT_ID")
        or default
    )


def load_oauth_access_token(project_ref: Optional[str] = None) -> Optional[str]:
    """Load OAuth access token from ~/.snipara/tokens.json.

    Returns:
        Access token string or None if not found/expired
    """
    project_ref = project_ref or resolve_snipara_project_ref()
