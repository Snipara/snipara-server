...
def load_oauth_token(project_slug: str = "snipara") -> dict | None:
    ...
    try:
        ...
        # Find token by project slug, explicit project ID, or token map key.
        for project_id, token_data in tokens.items():
            if (
                token_data.get("project_slug") == project_slug
                or token_data.get("project_id") == project_slug
                or project_id == project_slug
            ):
                # Check if expired
...
class ContextQueryResult:
    ...
    recommended_tool_arguments: dict[str, Any] | None = None
    graph_hybrid_used: bool = False
    graph_context_tool: str | None = None
...
class SniparaClient:
    ...
    def __init__(
        ...
        if self.api_key and (self.prefer_api_key or "api.snipara.com" not in self.base_url):
    ...
    async def _ensure_token_valid(self) -> None:
        ...
        print(
            f"\n    [oauth] Token expires in {remaining:.0f}s, refreshing...",
            file=sys.stderr,
            end="",
        )
        try:
            async with httpx.AsyncClient(timeout=30.0) as refresh_client:
                ...
                if resp.status_code != 200:
                    ...
                    if self.api_key:
                        print("    [oauth] Falling back to SNIPARA_API_KEY", file=sys.stderr)
                        self.access_token = None
    ...
    async def _get_client(self) -> httpx.AsyncClient:
        ...
        if self._client is None:
            ...
            if self.access_token:
                ...
                raise ValueError(
                    "No authentication configured. Run 'snipara-mcp-login' or set SNIPARA_API_KEY"
                )
    ...
    async def call_tool(self, tool_name: str, arguments: dict[str, Any] | None = None) -> dict:
        """Call an arbitrary MCP tool through the JSON-RPC API."""
        client = await self._get_client()
    ...
    def _parse_context_result(
        ...
        return ContextQueryResult(
            ...
            recommended_tool_arguments=result.get("recommended_tool_arguments"),
            graph_hybrid_used=result.get("graph_hybrid_used", False),
            graph_context_tool=result.get("graph_context_tool"),
            graph_context_summary=result.get("graph_context_summary"),
        )
    ...
    async def code_callers(self, **arguments: Any) -> dict:
        return await self.call_tool("rlm_code_callers", arguments)

    async def code_imports(self, **arguments: Any) -> dict:
        return await self.call_tool("rlm_code_imports", arguments)

    async def code_neighbors(self, **arguments: Any) -> dict:
        return await self.call_tool("rlm_code_neighbors", arguments)

    async def code_shortest_path(self, **arguments: Any) -> dict:
        return await self.call_tool("rlm_code_shortest_path", arguments)
