...
class SniparaClient:
    ...
    def _candidate_tool_urls(self) -> list[str]:
        ...
        Prefer the remote MCP backend endpoint and fall back to the web proxy
...
def _build_contract_handler(client: SniparaClient, tool_name: str):
    """Create a generic handler that forwards kwargs to a remote MCP tool."""
...
def _build_contract_tools(Tool: type["Tool"], client: SniparaClient, legacy_tools: list["Tool"]) -> list["Tool"]:
    """Expose the full remote tool contract alongside the legacy alias surface."""
