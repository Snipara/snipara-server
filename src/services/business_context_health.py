...
def normalize_business_project_lifecycle(value: Any) -> str | None:
    normalized = _string(value)
    if normalized in {"ACTIVE_CLIENT", "REFERENCE_ARCHIVE", "INTERNAL_BUSINESS"}:
        return normalized
    return None


def classify_project_scope(project: Any) -> str:
    if getattr(project, "githubRepo", None):
        return "code"

    lifecycle = normalize_business_project_lifecycle(getattr(project, "businessLifecycle", None))
    if lifecycle == "INTERNAL_BUSINESS":
        return "internal"
    if lifecycle in {"ACTIVE_CLIENT", "REFERENCE_ARCHIVE"}:
        return "client"

    description = _string(getattr(project, "description", None)) or ""
    for pattern, scope in _PROJECT_SCOPE_PREFIXES:
        if pattern.search(description):
            return scope

    searchable = f"{_string(getattr(project, 'name', None)) or ''} {description}".lower()
    if _RESEARCH_SCOPE_PATTERN.search(searchable):
        return "research"
    if _INTERNAL_SCOPE_PATTERN.search(searchable):
        return "internal"
    if _CODE_SCOPE_PATTERN.search(searchable):
        return "code"
    return "client"


def should_monitor_business_context_health(project: Any, documents: list[Any]) -> bool:
    scope = classify_project_scope(project)
    if scope in {"code", "research"}:
        return False

    lifecycle = normalize_business_project_lifecycle(getattr(project, "businessLifecycle", None))
    if lifecycle == "ACTIVE_CLIENT":
        return True
    if lifecycle in {"REFERENCE_ARCHIVE", "INTERNAL_BUSINESS"}:
        return False

    tracked_usage_modes: list[str] = []
    for document in documents:
        if _document_kind(document) == "CODE":
            continue
        metadata = _metadata_dict(getattr(document, "metadata", None))
        if not _asset_class(document, metadata):
            continue
        tracked_usage_modes.append(_usage_mode(metadata))

    if not tracked_usage_modes:
        return scope == "client"

    return any(mode == "current_truth" for mode in tracked_usage_modes)
