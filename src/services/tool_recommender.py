...
TOOL_METADATA: dict[str, ToolMetadata] = {
    ...
    "rlm_context_query": ToolMetadata(
        ...
        use_cases=[
            ...
            "Retry a timed-out broad query with narrower fast parameters",
        ],
        example=(
            'rlm_context_query(query="authentication middleware", max_tokens=1200, '
            'search_mode="keyword", return_references=True, auto_decompose=False)'
        ),
        related=["rlm_ask", "rlm_search", "rlm_code_neighbors"],
        description="Query documentation with semantic search, token budgeting, and fast retry parameters",
    ),
    ...
    "rlm_search": ToolMetadata(
        name="rlm_search",
        tier=ToolTier.PRIMARY,
        keywords=["search", "regex", "pattern", "find", "grep", "exact", "fallback", "timeout"],
        use_cases=[
            "Find exact text patterns",
            "Regex search in docs",
            "Fast fallback for a distinctive term after context query timeout",
        ],
    ...
    "rlm_memory_health": ToolMetadata(
        name="rlm_memory_health",
        tier=ToolTier.POWER_USER,
        keywords=["memory", "health", "hygiene", "diagnostics", "noise", "anomalies"],
        use_cases=[
            "Check memory health before or after cleanup",
            "Find noisy memory categories and active anomaly samples",
            "Verify auto-compaction thresholds without mutating memory",
        ],
        example="rlm_memory_health(sample_limit=5)",
        related=["rlm_memories", "rlm_memory_compact", "rlm_memory_daily_brief"],
        description="Read-only memory hygiene diagnostics",
    ),
    "rlm_memory_duplicate_candidates": ToolMetadata(
        name="rlm_memory_duplicate_candidates",
        tier=ToolTier.POWER_USER,
        keywords=["memory", "duplicate", "duplicates", "dedupe", "supersede", "review"],
        use_cases=[
            "Review duplicate memory groups before superseding anything",
            "Find candidate supersession groups without mutating memory",
        ],
        example="rlm_memory_duplicate_candidates(limit=10)",
        related=["rlm_memory_clean_candidates", "rlm_memory_compact", "rlm_memory_health"],
        description="Read-only duplicate and supersession candidate review",
    ),
    "rlm_memory_clean_candidates": ToolMetadata(
        name="rlm_memory_clean_candidates",
        tier=ToolTier.POWER_USER,
        keywords=["memory", "clean", "cleanup", "hygiene", "noise", "stale", "review"],
        use_cases=[
            "Inspect memory cleanup candidates grouped by risk",
            "Find noise, duplicates, stale memories, category anomalies, and review items",
        ],
        example="rlm_memory_clean_candidates(limit_per_bucket=10)",
        related=["rlm_memory_duplicate_candidates", "rlm_memory_health", "rlm_memory_compact"],
    ...
    "rlm_list_business_collections": ToolMetadata(
        name="rlm_list_business_collections",
        tier=ToolTier.TEAM,
        keywords=["business", "library", "collections", "offers", "presentations", "diagrams"],
        use_cases=[
            "List Team Business Context collections",
            "Find missing business presets before upload",
        ],
        example="rlm_list_business_collections(include_custom=False)",
        related=["rlm_ensure_business_collection", "rlm_upload_business_document"],
        description="List Team Business Context collections",
        requires_team=True,
    ),
    "rlm_ensure_business_collection": ToolMetadata(
        name="rlm_ensure_business_collection",
        tier=ToolTier.TEAM,
        keywords=["business", "collection", "create", "ensure", "playbook", "template", "diagram"],
        use_cases=[
            "Create Business Response Playbook, Business Library, or Offer Templates",
            "Bootstrap reference diagrams before upload",
        ],
        example='rlm_ensure_business_collection(preset="business_response_playbook")',
        related=["rlm_list_business_collections", "rlm_upload_business_document"],
        description="Create or return a Team Business Context collection",
        requires_team=True,
    ),
    "rlm_upload_business_document": ToolMetadata(
        ...
        use_cases=[
            "Upload reusable offer templates",
            "Add presentation notes or reference diagrams to business context",
        ],
        example='rlm_upload_business_document(preset="business_library", title="Offer Method", content="...")',
        related=["rlm_list_business_collections", "rlm_upload_document"],
        description="Upload reusable business documents to Team Business Context",
        requires_team=True,
    ),
    "rlm_list_client_projects": ToolMetadata(
        name="rlm_list_client_projects",
        tier=ToolTier.TEAM,
        keywords=["client", "project", "business", "context", "workspace", "list"],
        use_cases=[
            "Find the target client project before upload",
            "List client/project context workspaces",
        ],
        example="rlm_list_client_projects(limit=100)",
        related=["rlm_create_client_project", "rlm_upload_document"],
        description="List client/project business-context workspaces",
        requires_team=True,
    ),
    "rlm_create_client_project": ToolMetadata(
        name="rlm_create_client_project",
        tier=ToolTier.TEAM,
        keywords=["client", "project", "create", "workspace", "business", "context"],
        use_cases=[
            "Create a client workspace before uploading current documents",
            "Start a project-scoped business context",
        ],
        example='rlm_create_client_project(name="ACME Network Refresh", slug="acme-network-refresh")',
        related=["rlm_list_client_projects", "rlm_upload_document"],
    ...
    "rlm_upload_document": ToolMetadata(
        ...
        use_cases=[
            ...
            "Upload a binary parser document",
            "Add documentation file",
        ],
        example='rlm_upload_document(path="diagrams/network.vsdx", content="base64:...", kind="BINARY", format="vsdx")',
        related=["rlm_sync_documents"],
        description="Upload or update a project document, including supported binary parser files",
    ),
    "rlm_sync_documents": ToolMetadata(
        name="rlm_sync_documents",
        tier=ToolTier.UTILITY,
        keywords=["sync", "bulk", "documents", "batch", "upload", "binary", "business context"],
        use_cases=[
            "Batch upload documents",
            "Bulk upload business context files",
            "Sync documentation from CI/CD",
        ],
        example='rlm_sync_documents(documents=[{"path": "diagrams/network.vsdx", "content": "base64:...", "kind": "BINARY", "format": "vsdx"}])',
    ...
    "rlm_task_create": ToolMetadata(
        ...
        tier=ToolTier.ADVANCED,
        keywords=["task", "create", "queue", "swarm", "work", "htask", "compatibility"],
        use_cases=[
            "Create a simple queue-style work item backed by HierarchicalTask",
            "Add an N3 htask for agents without building a full hierarchy",
        ],
        example='rlm_task_create(swarm_id="...", title="Refactor auth")',
        related=["rlm_task_claim", "rlm_task_complete", "rlm_htask_create"],
        description="Compatibility alias that creates a canonical htask N3 work item",
        requires_admin=True,
    ),
    "rlm_task_claim": ToolMetadata(
        name="rlm_task_claim",
        tier=ToolTier.ADVANCED,
        keywords=["task", "claim", "take", "work", "queue", "htask", "compatibility"],
        use_cases=[
            "Claim a simple queue-style htask",
            "Start working on an N3 htask from the queue view",
        ],
        example='rlm_task_claim(swarm_id="...", agent_id="...")',
        related=["rlm_task_create", "rlm_task_complete", "rlm_htask_recommend_batch"],
        description="Compatibility alias that claims a canonical htask N3 task",
        requires_admin=True,
    ),
    "rlm_task_complete": ToolMetadata(
        name="rlm_task_complete",
        tier=ToolTier.ADVANCED,
        keywords=["task", "complete", "done", "finish", "result", "htask", "compatibility"],
        use_cases=[
            "Complete a simple queue-style htask",
            "Report a task result through the compatibility surface",
        ],
        example='rlm_task_complete(swarm_id="...", task_id="...", success=True)',
        related=["rlm_task_claim", "rlm_htask_complete"],
        description="Compatibility alias that completes or fails a canonical htask N3 task",
        requires_admin=True,
    ),
    ...
    "rlm_htask_create_feature": ToolMetadata(
        ...
        use_cases=[
            "Create a workflow feature with standard workstreams",
            "Turn roadmap work into N1/N2/N3 execution structure",
            "Plan multi-workstream implementation tasks",
        ],
        example='rlm_htask_create_feature(swarm_id="...", title="Memory lifecycle quality", owner="agent")',
        related=["rlm_htask_create", "rlm_htask_recommend_batch", "rlm_htask_tree"],
        description="Create a hierarchical feature with standard implementation workstreams",
    ),
    "rlm_htask_create": ToolMetadata(
        ...
        use_cases=[
            "Create hierarchical N0-N3 work items",
            "Capture acceptance criteria and evidence requirements",
            "Plan implementation tasks for a team or agent swarm",
        ],
        example='rlm_htask_create(swarm_id="...", title="Add contract test", owner="agent")',
        related=["rlm_htask_create_feature", "rlm_htask_recommend_batch"],
        description="Create a hierarchical task for durable workflow execution",
    ),
    "rlm_htask_recommend_batch": ToolMetadata(
        ...
        use_cases=[
            "Find the next ready N3 tasks in a workflow",
            "Prioritize unblocked backlog work",
            "Select an implementation batch from a planned initiative",
        ],
        example='rlm_htask_recommend_batch(swarm_id="...", limit=5)',
        related=["rlm_htask_tree", "rlm_htask_complete"],
        description="Recommend the next unblocked hierarchical tasks to execute",
    ),
    "rlm_htask_tree": ToolMetadata(
        name="rlm_htask_tree",
        tier=ToolTier.ADVANCED,
        keywords=[
            "workflow",
        ...
        use_cases=[
            "Inspect a full hierarchical workflow tree",
            "Review roadmap status across initiatives and features",
            "Understand dependencies before closing or executing tasks",
        ],
        example='rlm_htask_tree(swarm_id="...", task_id="...")',
        related=["rlm_htask_get", "rlm_htask_recommend_batch"],
        description="Load the hierarchical task tree for workflow review",
    ),
    "rlm_htask_complete": ToolMetadata(
        ...
        use_cases=[
            "Complete a leaf workflow task with evidence",
            "Record implementation result and learnings",
            "Create a memory from a completed task outcome",
        ],
        example='rlm_htask_complete(swarm_id="...", task_id="...", evidence=[...])',
        related=["rlm_end_of_task_commit", "rlm_htask_recommend_batch"],
        description="Complete an N3 workflow task with evidence and optional memory",
    ),
    "rlm_htask_verify_closure": ToolMetadata(
        ...
        use_cases=[
            "Check whether a parent workflow can close",
            "Find incomplete or blocked children before closure",
            "Validate acceptance criteria across a feature or initiative",
        ],
        example='rlm_htask_verify_closure(swarm_id="...", task_id="...")',
        related=["rlm_htask_close", "rlm_htask_tree"],
        description="Verify hierarchical workflow closure readiness",
    ),
    "rlm_htask_close": ToolMetadata(
        name="rlm_htask_close",
        tier=ToolTier.ADVANCED,
        keywords=["workflow", "close", "closure", "waiver", "finish", "roadmap", "htask"],
        use_cases=[
            "Close a completed parent workflow item",
            "Close with a waiver when policy allows",
        ],
        example='rlm_htask_close(swarm_id="...", task_id="...")',
        related=["rlm_htask_verify_closure", "rlm_htask_tree"],
        description="Close a parent hierarchical workflow item",
    ),
...
def _compute_intent_boost(metadata: ToolMetadata, query_terms: list[str], query: str) -> float:
    ...
    workflow_orchestration_intent = len(terms & workflow_terms) >= 2 or _contains_phrase(
        query,
        [
            "plan all this",
            "planifier tout",
    ...
    implementation_planning_intent = (
        len(terms & implementation_terms) >= 2
        and bool(terms & {"plan", "planning", "workflow", "roadmap", "tasks"})
    ) or _contains_phrase(
        query,
        [
            "repo work",
    ...
    session_bootstrap_intent = len(
        terms & {"session", "resume", "bootstrap", "recall"}
    ) >= 2 or _contains_phrase(
        query,
        [
            "session bootstrap",
            "session start",
    ...
    workspace_profile_intent = len(
        terms & {"workspace", "tenant", "profile", "constraints"}
    ) >= 2 or _contains_phrase(query, ["workspace profile", "tenant profile", "client profile"])
    durable_commit_intent = len(
        terms & {"persist", "durable", "knowledge", "commit", "summary"}
    ) >= 2 or _contains_phrase(
        query,
        [
            "end of task",
    ...
    dedupe_intent = bool(
        terms & {"duplicate", "duplicates", "dedupe", "novel", "novelty"}
    ) or _contains_phrase(query, ["without duplicates", "avoid duplicates", "if novel"])
    daily_brief_intent = len(
        terms & {"daily", "brief", "active", "constraints", "todo"}
    ) >= 2 or _contains_phrase(query, ["daily brief", "active constraints"])
    index_health_intent = len(
        terms & {"index", "coverage", "chunks", "stale", "health", "quality"}
    ) >= 2 or _contains_phrase(
        query,
        [
            "index health",
    ...
    if timeout_recovery_intent:
        if metadata.name == "rlm_context_query":
            boost += 75.0
        elif metadata.name == "rlm_search":
            boost += 30.0
            ...
            boost += 20.0
        elif metadata.name == "rlm_decompose":
            boost -= 8.0

    if workflow_orchestration_intent:
        if metadata.name == "rlm_plan":
            boost += 70.0
        elif metadata.name == "rlm_htask_recommend_batch":
            boost += 65.0
        elif metadata.name == "rlm_htask_create_feature":
            boost += 58.0
        elif metadata.name == "rlm_htask_tree":
            boost += 42.0
        elif metadata.name == "rlm_orchestrate":
            boost += 38.0
        elif metadata.name == "rlm_decompose":
            boost += 18.0
        elif metadata.name in {"rlm_task_create", "rlm_task_claim", "rlm_task_complete"}:
            boost -= 10.0
        elif metadata.name in {"rlm_search", "rlm_read", "rlm_get_chunk"}:
            boost -= 15.0

    if implementation_planning_intent:
        if metadata.name == "rlm_plan":
            boost += 45.0
        elif metadata.name in {
            "rlm_code_neighbors",
            ...
            boost += 22.0
        elif metadata.name == "rlm_context_query":
            boost += 12.0

    if workflow_orchestration_intent and has_memory_intent:
        if metadata.name in {
