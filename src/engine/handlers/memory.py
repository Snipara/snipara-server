...
async def handle_remember_if_novel(
    ...
    document_refs = params.get("document_refs")
    agent_id = params.get("agent_id")
    novelty_threshold = normalize_memory_novelty_threshold(
        params.get(
            "novelty_threshold",
            getattr(ctx.settings, "memory_novelty_threshold", None),
        )
    )
...
async def handle_session_bootstrap_status(
    params: dict[str, Any],
    ctx: HandlerContext,
) -> ToolResult:
    """Return read-only session bootstrap status supplied by the caller context."""
    del ctx
    status = params.get("bootstrap") or {
        "ran": False,
        "timestamp": None,
...
async def handle_memory_health(
    params: dict[str, Any],
    ctx: HandlerContext,
) -> ToolResult:
    """Return read-only memory health and hygiene counters."""
...
async def handle_memory_duplicate_candidates(
    params: dict[str, Any],
    ctx: HandlerContext,
) -> ToolResult:
    """Return read-only duplicate/supersession candidates."""
...
async def handle_memory_clean_candidates(
    params: dict[str, Any],
    ctx: HandlerContext,
) -> ToolResult:
