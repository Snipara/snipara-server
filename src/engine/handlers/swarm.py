...
from ...services.htask_task_adapter import (
    claim_task_as_htask,
    complete_task_as_htask,
    create_task_as_htask,
    create_tasks_bulk_as_htask,
    get_task_stats_as_htask,
    list_task_events_as_htask,
    list_tasks_as_htask,
    recover_stuck_tasks_as_htask,
    unclaim_task_as_htask,
)
...
async def handle_task_create(
    ...
    """Create a queue-style task as a canonical htask N3 item.
...
async def handle_task_bulk_create(
    params: dict[str, Any],
    ctx: HandlerContext,
) -> ToolResult:
    """Create multiple tasks through the legacy-compatible task surface."""
    swarm_id = params.get("swarm_id", "")
    agent_id = params.get("agent_id", "")
    ...
    result = await create_tasks_bulk_as_htask(
        swarm_id=swarm_id,
        agent_id=agent_id,
        tasks=tasks,
    )
    ...
    input_tokens = sum(
        count_tokens(task.get("title", "") + (task.get("description") or "")) for task in tasks
    )
    return ToolResult(
        data=result,
...
async def handle_task_claim(
    ...
    result = await claim_task_as_htask(
        swarm_id=swarm_id,
        agent_id=agent_id,
        task_id=task_id,
    )
...
async def handle_task_complete(
    ...
    result = await complete_task_as_htask(
        swarm_id=swarm_id,
        agent_id=agent_id,
        task_id=task_id,
        success=success,
...
async def handle_tasks(
    ...
    result = await list_tasks_as_htask(
        swarm_id=swarm_id,
        status=status,
...
async def handle_task_stats(
    ...
    result = await get_task_stats_as_htask(swarm_id=swarm_id)
...
async def handle_task_events(
    ...
    result = await list_task_events_as_htask(
        swarm_id=swarm_id,
        since=since,
        limit=limit,
    )
...
async def handle_task_unclaim(
    ...
    result = await unclaim_task_as_htask(
        swarm_id=swarm_id,
        task_id=task_id,
...
async def handle_task_recover(
    ...
    result = await recover_stuck_tasks_as_htask(
        swarm_id=swarm_id,
