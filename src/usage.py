...
def should_apply_plan_ip_rate_limit(plan: str | None) -> bool:
    """Return whether the shared IP limiter should apply to an authenticated plan."""
    plan_value = getattr(plan, "value", plan)
    return plan_value == "FREE"


async def check_plan_ip_rate_limit(client_ip: str | None, plan: str | None) -> bool:
    """Apply the shared IP limiter only to plans that need an aggregate free-tier cap."""
    if not client_ip or not should_apply_plan_ip_rate_limit(plan):
        return True
    return await check_ip_rate_limit(client_ip)
...
async def track_usage(
    ...
    """
    Track a query for usage analytics and enterprise quota accounting.
