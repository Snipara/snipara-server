...
class IPRateLimitMiddleware:
    ...
    Applies to public HTTP requests before reaching endpoint handlers.
    Uses X-Forwarded-For header (behind reverse proxy) or direct client address.
    Skips authenticated API surfaces so plan-aware rate limits can run after auth.
    """
    ...
    async def __call__(self, scope, receive, send):
        ...
        # Skip public metadata endpoints that browsers and uptime checks may fetch often.
        path = scope.get("path", "")
        if path in ("/", "/favicon.ico", "/health", "/ready", "/docs", "/redoc", "/openapi.json"):
            await self.app(scope, receive, send)
            return
        ...
        if path == "/mcp" or path.startswith("/mcp/") or path == "/v1" or path.startswith("/v1/"):
            await self.app(scope, receive, send)
            return
